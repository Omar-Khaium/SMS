...
session_start();
    if(isset($_SESSION["flag"]) && isset($_REQUEST["Batch"]) && $_SESSION["flag"]=="success"){
        if(isset($_REQUEST["Class"]) && isset($_REQUEST["Batch"]))
        {
            $sql = "select * from batch where Class='".$_REQUEST["Class"]."' AND  `Batch ID`='".$_REQUEST["Batch"]."';";
   ...
   <!-------------------------------navigation start--------------------------------------->
    ...
    <script>
        ...
        function updateClass(element)
        {
            ...
            window.location = "Attendence_Form_Class.php?Batch="+BatchValue;
        }
        function  Check()
        {
