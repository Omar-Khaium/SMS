...
session_start();
    ...
    $sql = "select * from applcation where Status='Pending';";
    ...
    <table style="margin-top: 50px;margin-left: 50px;width:90%">
        ...
        if(count($result)==0)
        {
            ...
            <div class="w3-bar-item">
                <span style="color:red;cursor: pointer;">No pending Application</span>
            </div>
                </li>';
        }
        else
        {
            ...
            {
                echo '
                <li class="w3-bar" id="Show-'.$data["Serial"].'">
                <div class="w3-bar-item">
                <span style="color:red;cursor: pointer;" id="Serial-'.$data["Serial"].'" class="w3-xlarge" onclick="showDesc(this)">'.$data["Subject"].'</span><br>'; 
                $sql = "select Name from tcr_profile where Id='".$data["User Id"]."';"; $teacher = getDataFromDB($sql);
                echo '
                <span class="w3-large">'.$teacher[0]["Name"].'</span><br>
                <span>'.$data["User Id"].'</span>
                </div>
                <div style="float:right;margin-top:3%">
                    <input onclick="TakeAction(this)" class="w3-radio" type="radio" value="Approve" name="'.$data["Serial"].'-'.$data["Application Id"].'" id="'.$data["Serial"].'-'.$data["Application Id"].'"><span style="color:green;"><b>Approve</b></span>&nbsp;&nbsp;&nbsp;&nbsp;
                    <input onclick="TakeAction(this)" class="w3-radio" type="radio" value="Reject" name="'.$data["Serial"].'-'.$data["Application Id"].'" id="'.$data["Serial"].'-'.$data["Application Id"].'"><span style="color:red;"><b>Reject</b></span>
                </div>
                </li>
                '; 
            }
    ...
    <script>
        function TakeAction(element) {
            ...
            if(element.value=="Approve"){
                var url = "submitAction.php?id="+arr[1]+"&status=1";
            }
            else if(element.value=="Reject"){
                var url = "submitAction.php?id="+arr[1]+"&status="+2;
            }
